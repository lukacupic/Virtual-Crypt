...
class ReflectionsPass extends Pass {
  constructor(ssrEffect, options = {}) {
    ...
    const width =
      options.width || typeof window !== "undefined" ? window.innerWidth : 2000;
    const height =
  ...
  unsetMRTMaterialInScene() {
    for (const c of this.visibleMeshes) {
      ...
      if (
        ((_c$material = c.material) == null ? void 0 : _c$material.type) ===
        "MRTMaterial"
      ) {
...
class TemporalResolveMaterial extends ShaderMaterial {
  constructor(customComposeShader) {
    const fragmentShader = temporalResolve.replace(
      "#include <custom_compose_shader>",
      customComposeShader
    );
    super({
...
class SSREffect extends Effect {
  ...
  constructor(scene, camera, options = defaultSSROptions) {
    ...
    this.uniforms.get("reflectionsTexture").value =
      this.temporalResolvePass.renderTarget.texture; // reflections pass
  ...
  update(renderer, inputBuffer) {
    ...
    if (this._camera.setViewOffset)
      this._camera.setViewOffset(width, height, x, y, width, height); // render reflections of current frame
